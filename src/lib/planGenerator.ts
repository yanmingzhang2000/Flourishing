...
import { ExerciseData, Exercise } from './types';
...
export function generateWeeklyPlan(profile: UserProfile, weekNumber: number = 1): WeeklyPlan {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    if (dayType === 'rest') {
      ...
      const strengthExercises = selectStrengthExercises(exercises, profile);
      days.push({
        day: DAY_NAMES[(date.getDay()) % 7 === 0 ? 7 : date.getDay()],
        dayIndex: i,
        type: 'strength',
        exercises: strengthExercises,
        warmup: exercises.warmup.slice(0, 2),
        cooldown: exercises.cooldown.slice(0, 2),
      });
    } else {
      days.push({
        day: DAY_NAMES[(date.getDay()) % 7 === 0 ? 7 : date.getDay()],
...
function selectStrengthExercises(exercises: ExerciseData, profile: UserProfile): WorkoutExercise[] {
